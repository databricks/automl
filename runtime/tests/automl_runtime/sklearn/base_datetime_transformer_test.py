    ...
    def setUp(self) -> None:
        ...
        self.X = pd.concat([
            pd.Series(range(num_rows), name='timestamp1').apply(
                lambda i: pd.Timestamp(2017, 2, 1, i + 5)),
        ],
                           axis=1)

    def test_cyclic_transform(self):
        cyclic_transformed = pd.concat(
            BaseDatetimeTransformer._cyclic_transform(
                self.X['timestamp1'].dt.hour, 24),
            axis=1)
        ...
        np.testing.assert_array_almost_equal(
            ...
            err_msg=
            f"Actual: {cyclic_transformed}\nExpected: {cyclic_expected}\n"
            f"Equality: {cyclic_transformed == cyclic_expected}")

    def test_generate_datetime_features(self):
        feature_generated = BaseDatetimeTransformer._generate_datetime_features(
            self.X[['timestamp1']]).to_numpy()
        feature_expected = np.array([
            ...
            [
                ...
                0.8498625371243979, 0.8660254037844387, -0.4999999999999998, 8,
                False, False
            ],
        ])
        np.testing.assert_array_almost_equal(
