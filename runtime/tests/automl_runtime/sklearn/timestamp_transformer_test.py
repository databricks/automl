...
class TestTimestampTransformer(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.X = pd.concat([
            ...
            pd.Series(range(num_rows), name="timestamp_str").apply(
                lambda i: f"2020-07-0{i+1} 01:23:45"),
        ],
                           axis=1)
        self.timestamp_expected = np.array([
            [
                ...
                0.8509914765261879, 0.9659258262890683, 0.25881904510252074, 5,
                False, False
            ],
            [
    ...
    def test_transform(self):
        timestamp_transformed = self.transformer.transform(
            self.X[["timestamp1"]])
        np.testing.assert_array_almost_equal(
            timestamp_transformed.to_numpy(),
        ...
        timestamp_str_transformed = self.transformer.transform(
            self.X[["timestamp_str"]])
        np.testing.assert_array_almost_equal(
            timestamp_str_transformed.to_numpy(),
            self.timestamp_str_expected,
            decimal=5,
