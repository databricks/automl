...
class TestProphetModelWithCovariates(BaseProphetModelTest):
    @classmethod
    def setUpClass(cls) -> None:
        num_rows = 9
        cls.X = pd.concat(
            [
                ...
                pd.Series(range(num_rows), name="y"),
                pd.Series(range(num_rows), name="covariate1"),
                pd.Series([i * 2 for i in range(num_rows)], name="covariate2"),
            ],
            axis=1,
        )
        cls.model_json = PROPHET_MODEL_JSON

    def test_predict_timeseries_with_covariates(self):
        prophet_model = ProphetModel(self.model_json, 3, "d", 1, "ds")
        
        # Create future dataframe with covariates
        future_df = pd.DataFrame({
            "ds": pd.date_range("2020-10-26", periods=5, freq="3D"),
            "covariate1": [10, 11, 12, 13, 14],
            "covariate2": [20, 22, 24, 26, 28]
        })
        
        # Test prediction with future dataframe containing covariates
        forecast_pd = prophet_model.predict_timeseries(future_df=future_df)
        expected_columns = {"yhat", "yhat_lower", "yhat_upper"}
        self.assertTrue(expected_columns.issubset(set(forecast_pd.columns)))
        self.assertEqual(len(forecast_pd), 5)

    def test_predict_with_covariates(self):
        prophet_model = ProphetModel(self.model_json, 1, "d", 1, "ds")
        
        test_df = pd.DataFrame({
            "ds": [pd.to_datetime("2020-10-08"), pd.to_datetime("2020-12-10")],
            "covariate1": [1, 2],
            "covariate2": [3, 4]
        })
        expected_test_df = test_df.copy()
...
class TestProphetModelFrequencyQuantities(BaseProphetModelTest):
    @classmethod
    def setUpClass(cls) -> None:
        cls.model_json = PROPHET_MODEL_JSON
        cls.frequency_quantities = [1, 5, 10, 15, 30]

    def test_make_future_dataframe_various_frequencies(self):
        """Test make_future_dataframe with various frequency quantities for minutes"""
        for frequency_quantity in self.frequency_quantities:
            prophet_model = ProphetModel(self.model_json, 2, "min", frequency_quantity, "ds")
            future_df = prophet_model.make_future_dataframe(horizon=2, include_history=False)
            self.assertEqual(len(future_df), 2)
            
            # Check that the time difference matches the expected frequency
            if len(future_df) > 1:
                time_diff = future_df.iloc[1]["ds"] - future_df.iloc[0]["ds"]
                expected_diff = pd.Timedelta(minutes=frequency_quantity)
                self.assertEqual(time_diff, expected_diff)

    def test_predict_timeseries_various_frequencies(self):
        """Test predict_timeseries with various frequency quantities"""
        for frequency_quantity in self.frequency_quantities:
            prophet_model = ProphetModel(self.model_json, 1, "min", frequency_quantity, "ds")
            forecast_pd = prophet_model.predict_timeseries(horizon=1, include_history=False)
            self.assertEqual(len(forecast_pd), 1)


class TestProphetModelEdgeCases(BaseProphetModelTest):
    @classmethod
    def setUpClass(cls) -> None:
        cls.model_json = PROPHET_MODEL_JSON

    def test_predict_with_empty_future_df(self):
        """Test predict_timeseries with None future_df"""
        prophet_model = ProphetModel(self.model_json, 2, "d", 1, "ds")
        forecast_pd = prophet_model.predict_timeseries(future_df=None, include_history=False)
        self.assertEqual(len(forecast_pd), 2)

    def test_predict_timeseries_horizon_override(self):
        """Test that horizon parameter overrides model's default horizon"""
        prophet_model = ProphetModel(self.model_json, 1, "d", 1, "ds")  # default horizon = 1
        forecast_pd = prophet_model.predict_timeseries(horizon=3, include_history=False)
        self.assertEqual(len(forecast_pd), 3)  # Should use provided horizon, not default

    def test_predict_with_mixed_date_types(self):
        """Test predict with mixed datetime types (string, datetime, date)"""
        prophet_model = ProphetModel(self.model_json, 1, "d", 1, "ds")
        ...
        test_df = pd.DataFrame({
            "ds": [
                "2020-10-08",  # string
                pd.to_datetime("2020-10-09"),  # datetime
    ...
    def test_model_serialization_properties(self):
        """Test that model properties are correctly set"""
        prophet_model = ProphetModel(
        ...
        self.assertEqual(prophet_model._horizon, 5)
        self.assertEqual(prophet_model._frequency_unit, "h")
        self.assertEqual(prophet_model._frequency_quantity, 2)
        self.assertEqual(prophet_model._time_col, "timestamp")
        self.assertEqual(prophet_model._split_col, "split_col")
        self.assertIsNotNone(prophet_model._preprocess_func)


class TestMultiSeriesProphetModelEdgeCases(BaseProphetModelTest):
    @classmethod
    def setUpClass(cls) -> None:
        cls.model_json = PROPHET_MODEL_JSON
        cls.multi_series_model_json = {("1",): cls.model_json, ("2",): cls.model_json}
    ...
    def test_predict_with_missing_model_for_id(self):
        """Test prediction when a model doesn't exist for a given ID"""
        model_json_subset = {("1",): self.model_json}  # Only model for ID "1"
        start_subset = {("1",): pd.Timestamp("2020-07-01")}
        
        prophet_model = MultiSeriesProphetModel(
            model_json=model_json_subset,
            timeseries_starts=start_subset,
            timeseries_end="2020-07-25",
            horizon=1,
            frequency_unit="days",
            frequency_quantity=1,
        ...
        test_df = pd.DataFrame({
            ...
            "id": ["2"]  # ID that doesn't have a model
        })
        
        yhat = prophet_model.predict(None, test_df)
        self.assertEqual(1, len(yhat))
        self.assertTrue(pd.isna(yhat.iloc[0]))  # Should return NaN for missing model

    def test_make_future_dataframe_with_specific_groups(self):
        """Test make_future_dataframe with specific groups parameter"""
        prophet_model = MultiSeriesProphetModel(
            model_json=self.multi_series_model_json,
            timeseries_starts=self.multi_series_start,
            timeseries_end="2020-07-25",
        ...
        future_df = prophet_model.make_future_dataframe(groups=[("1",)], include_history=False)
        self.assertEqual(len(future_df), 1)
        self.assertTrue((future_df["id"] == "1").all())

    def test_get_reserved_cols(self):
        """Test that get_reserved_cols returns expected columns"""
        reserved_cols = MultiSeriesProphetModel.get_reserved_cols()
        expected_base_cols = ["trend", "additive_terms", "daily", "weekly", "yearly",
                             "holidays", "zeros", "extra_regressors_additive", "yhat",
                             "extra_regressors_multiplicative", "multiplicative_terms"]
        
        # Check that all base columns are present
        for col in expected_base_cols:
            self.assertIn(col, reserved_cols)
        
        # Check that _lower and _upper variants are present
        for col in expected_base_cols:
            self.assertIn(f"{col}_lower", reserved_cols)
            self.assertIn(f"{col}_upper", reserved_cols)
        
        # Check additional expected columns
        additional_cols = ["y", "cap", "floor", "y_scaled", "cap_scaled"]
        for col in additional_cols:
            self.assertIn(col, reserved_cols)

    def test_model_predict_function(self):
        """Test the model_predict function specifically"""
        prophet_model = MultiSeriesProphetModel(
            model_json=self.multi_series_model_json,
            timeseries_starts=self.multi_series_start,
            timeseries_end="2020-07-25",
        ...
        test_df = pd.DataFrame({
            "ds": [pd.to_datetime("2020-07-26")],
            "id": ["1"]
        })
        
        result_df = prophet_model.model_predict(test_df, horizon=1)
        
        # Check that result has expected columns
        expected_cols = MultiSeriesProphetModel.get_reserved_cols() + ["ds"] + ["id"]
        for col in expected_cols:
            self.assertIn(col, result_df.columns)


class TestProphetModelQuarterlyFrequency(BaseProphetModelTest):
    """Test quarterly frequency which was previously disabled"""
    
    @classmethod
    def setUpClass(cls) -> None:
        cls.model_json = PROPHET_MODEL_JSON

    def test_quarterly_frequency_detection(self):
        """Test that quarterly frequency is properly detected"""
        from databricks.automl_runtime.forecast.utils import is_quaterly_alias
        
        # Test various quarterly aliases
        quarterly_aliases = ['Q', 'QS', 'quarter', 'quarterly']
        for alias in quarterly_aliases:
            if alias in ['Q', 'QS']:  # Only test valid pandas frequency aliases
                prophet_model = ProphetModel(self.model_json, 1, alias, 1, "ds")
                # Just test that the model can be created without error
                self.assertIsNotNone(prophet_model)

    def test_monthly_frequency_support(self):
        """Test monthly frequency support"""
        prophet_model = ProphetModel(self.model_json, 1, "MS", 1, "ds")
        
        # Test make_future_dataframe with monthly frequency
        future_df = prophet_model.make_future_dataframe(horizon=2, include_history=False)
        self.assertEqual(len(future_df), 2)
        
        # Verify the time difference is approximately one month
        if len(future_df) > 1:
            time_diff = future_df.iloc[1]["ds"] - future_df.iloc[0]["ds"]
            # Monthly difference should be between 28-31 days
            self.assertTrue(28 <= time_diff.days <= 31)


class TestProphetModelSignatureInference(BaseProphetModelTest):
    """Test model signature inference functionality"""
    
    @classmethod
    def setUpClass(cls) -> None:
        cls.model_json = PROPHET_MODEL_JSON

    def test_infer_signature_default(self):
        """Test signature inference with default sample input"""
        prophet_model = ProphetModel(self.model_json, 1, "d", 1, "ds")
        signature = prophet_model.infer_signature()
        self.assertIsNotNone(signature)

    def test_infer_signature_custom_input(self):
        """Test signature inference with custom sample input"""
        prophet_model = ProphetModel(self.model_json, 1, "d", 1, "time_column")
        
        sample_input = pd.DataFrame({
            "time_column": [pd.to_datetime("2020-10-01"), pd.to_datetime("2020-10-02")],
            "covariate": [1, 2]
        })
